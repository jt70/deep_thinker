...
fn main() {
    ...
    let start_e: f32 = 1.0;
    let end_e: f32 = 0.05;
    let exploration_fraction: f32 = 0.5;
    let learning_starts = 10_000;
    let train_frequency = 10;
    //let batch_size = 5;
    let batch_size = 128;
    let learning_rate: LearningRate = 0.00025;
    let target_network_frequency = 500;
    let gamma = Tensor::<MyAutodiffBackend, 2>::full([batch_size, 1], 0.99);
    let ones = Tensor::<MyAutodiffBackend, 2>::full([batch_size, 1], 1.0);

    // TODO: get input/output size from environment
    let num_actions = 2;
    let random_action_sampler = Uniform::from(0..num_actions as i32);
